 ...
 * one record per line to stdout/stderr. In production the record is JSON so a
 * log shipper can parse it; in development it is a short human-readable line.
 *
...
function emit(level: LogLevel, msg: string, fields?: LogFields): void {
  ...
  if (process.env.NODE_ENV === "production") {
    sink(JSON.stringify({ level, time: new Date().toISOString(), msg, ...safe }));
    return;
  }
