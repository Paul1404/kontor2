...
import { Building2, ExternalLink, Loader2, Plus, Power, PowerOff, Trash2 } from "lucide-react";
...
export const Route = createFileRoute("/app/admin/vereine")({
  component: VereinePage,
});
...
function VereinePage() {
  ...
  const list = useQuery({ queryKey: ["tenants"], queryFn: () => orpc.tenants.list() });
  ...
  const create = useMutation({
    mutationFn: () => orpc.tenants.create({ key: key.trim().toLowerCase(), displayName }),
    onSuccess: () => {
      setError(null);
      setKey("");
      setDisplayName("");
  ...
  const setStatus = useMutation({
    mutationFn: (input: { key: string; status: "active" | "disabled" }) =>
      orpc.tenants.setStatus(input),
    onSuccess: refresh,
    onError: (err) => setError((err as Error).message),
  });

  const remove = useMutation({
    mutationFn: (input: { key: string; confirmKey: string }) => orpc.tenants.remove(input),
    onSuccess: () => {
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-6 p-1">
      <div className="flex items-center gap-3">
        <Building2 className="size-6 text-brand" />
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Vereine</h1>
          <p className="text-sm text-muted-foreground">
            Mandanten anlegen und verwalten. Nur im Hauptverein verfügbar.
          </p>
        </div>
      </div>

      <InfoBox>
        Jeder Verein bekommt eine eigene Datenbank und eine eigene Subdomain (
        <code>name.{domain}</code>). Beim Anlegen werden Datenbank, alle Aktualisierungen und ein
        eigener Verschlüsselungsschlüssel automatisch eingerichtet. Der erste Admin entsteht dann
        über <code>/setup</code> auf der Subdomain des Vereins.
      </InfoBox>
      ...
      <Card>
        ...
        <CardContent>
          {list.isError ? (
            ...
            <p className="text-sm text-muted-foreground">
              Noch keine weiteren Vereine. Der Hauptverein ({orEmpty(list.data?.primaryKey)}) wird
                  ...
                  {list.data?.tenants.map((t) => (
                    <tr key={t.key} className="border-b last:border-0">
                      <td className="py-2.5 pr-3">
                        <div className="font-medium">{orEmpty(t.displayName)}</div>
                        <a
                      ...
                      <td className="py-2.5">
                        <div className="flex justify-end gap-1">
                          <Button
                            ...
                            title={t.status === "active" ? "Sperren" : "Entsperren"}
                          >
                            {t.status === "active" ? (
